...
public interface SwapTransactionRepository extends JpaRepository<SwapTransaction, String> {
    ...
    @Query(value = "select sum(total_value) as volume, sum(amount_a) as volumeAmount  from swap_transaction where token_a = :token "
    ...
    @Query(value = "select sum(total_value) as volume, sum(amount_b) as volumeAmount  from swap_transaction where token_b = :token "
            + "and swap_type > 0 and swap_type < 3 and (ts between :start_time and :end_time)", nativeQuery = true)
    TokenVolumeDTO getVolumeByTokenB(@Param("token") String tokenB, @Param("start_time") long startTime, @Param("end_time") long endTime);

    @Query(value = "select sum(total_value) as volume, sum(amount_a) as volumeAmount  from swap_transaction where token_a = :tokenA "
            + "and token_b = :tokenB "
            + " and swap_type > 0 and swap_type < 3 and (ts between :start_time and :end_time)", nativeQuery = true)
    ...
    @Query(value = "select sum(total_value) as volume, sum(amount_b) as volumeAmount  from swap_transaction where token_b = :tokenA "
            + "and token_a = :tokenB "
            + " and swap_type > 0 and swap_type < 3 and (ts between :start_time and :end_time)", nativeQuery = true)
